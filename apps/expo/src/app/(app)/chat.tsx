...
import { View, Text, TextInput, FlatList, Pressable, Platform, ActivityIndicator } from "react-native";
...
const notifications = [
    { id: 1, points: 150, transactionDate: "2024-10-01", from: "João", to: "Você" },
    { id: 2, points: 200, transactionDate: "2024-10-03", from: "Maria", to: "Você" },
    { id: 3, points: 50, transactionDate: "2024-10-05", from: "Você", to: "Pedro" },
...
export default function ChatbotScreen() {
  ...
  const sendMessage = async () => {
    ...
    setInputText("");
    const transactionDetails = notifications.map(
      (notification) =>
  ...
  const renderMessage = ({ item }: { item: Message }) => (
    ...
    >
      <Text className={item.isUser ? "text-white" : "text-black"}>{item.text}</Text>
    </View>
  );
  return (
    <View className="flex-1 bg-gray-100">
      ...
      <View className="flex-row items-center bg-white p-4">
        <TextInput
          className="flex-1 rounded-full border border-gray-300 px-4 py-2 mr-2"
          value={inputText}
          ...
          editable={!isPending}
        />
        <Pressable onPress={sendMessage} className="rounded-full bg-blue-500 p-2" disabled={isPending}>
          {isPending ? (
