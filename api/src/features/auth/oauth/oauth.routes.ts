...
export const handleOAuthRequest = async (
  ...
  if (/^\/auth\/(github|google|apple)\/callback$/.test(url.pathname)) {
    ...
    res.setHeader("Set-Cookie", serializeCookie("session", result.session.id))

    const redirectUrl = new URL(redirect)
    redirectUrl.searchParams.append("token", result.session.id)
  ...
  if (
    ...
    res.end(JSON.stringify({ token: result.session.id }))
    return
  }
